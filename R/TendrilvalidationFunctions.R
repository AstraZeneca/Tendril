...
validate.tendril.data <- function(mydata, rotations, Treatments, Terms, Unique.Subject.Identifier, Treat, StartDay, AEfreqTreshold){
  if (!"data.frame" %in% class(mydata)){
    stop("The dataset is not a dataframe")
  }
  if (dim(mydata)[1] != length(rotations) | !is.numeric(rotations)){
    stop("Rotations must be anumeric vectors of length equal to the number of rows in mydata")
  }
